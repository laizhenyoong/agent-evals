...
from evals.types import RunRecord, ToolCall


class Stage3Tests(unittest.TestCase):
    def test_golden_set_has_fifty_cases_with_unique_ids(self) -> None:
        self.assertEqual(len(GOLDEN_CASES), 50)
        self.assertEqual(len({case.id for case in GOLDEN_CASES}), 50)

    def test_deterministic_scorer_accepts_required_call_and_arguments(self) -> None:
        case = next(case for case in GOLDEN_CASES if case.id == "account_alice_plan")
        run = RunRecord(case.id, "Alice is on Pro.", (ToolCall("check_account", {"account_id": "A123"}),))
    ...
    def test_every_golden_contract_has_a_passing_reference_trajectory(self) -> None:
        for case in GOLDEN_CASES:
            calls = tuple(
                ToolCall(expectation.name, expectation.exact_args | expectation.contains_args)
                for expectation in case.required_tools
            )
            run = RunRecord(case.id, "reference answer", calls)
    ...
    def test_deterministic_scorer_rejects_wrong_argument_and_loop(self) -> None:
        case = next(case for case in GOLDEN_CASES if case.id == "account_alice_plan")
        run = RunRecord(case.id, "", (ToolCall("check_account", {"account_id": "B456"}), ToolCall("check_account", {"account_id": "A123"})))
    ...
    def test_trajectory_scorer_rejects_correct_but_wasteful_path(self) -> None:
        case = next(case for case in GOLDEN_CASES if case.id == "account_alice_plan")
        run = RunRecord(case.id, "Alice is on Pro.", (ToolCall("search_kb", {"query": "plan"}), ToolCall("check_account", {"account_id": "A123"})))
